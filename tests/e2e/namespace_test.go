...
func TestNamespaceCreation(t *testing.T) {
	...
	var temporalNamespaceWithCustomSearchAttributes *v1beta1.TemporalNamespace
	...
	namespaceFeature := features.New("namespace creation using CRD").
		...
		Assess("Can create a temporal namespace with custom search attributes", func(ctx context.Context, _ *testing.T, cfg *envconf.Config) context.Context {
			namespace := GetNamespaceForFeature(ctx)

			// create the temporal cluster client
			temporalNamespaceWithCustomSearchAttributes = &v1beta1.TemporalNamespace{
				ObjectMeta: metav1.ObjectMeta{Name: "test-search-attr", Namespace: namespace},
				Spec: v1beta1.TemporalNamespaceSpec{
					ClusterRef: v1beta1.ObjectReference{
						Name: cluster.GetName(),
					},
			...
			err := cfg.Client().Resources(namespace).Create(ctx, temporalNamespaceWithCustomSearchAttributes)
			if err != nil {
				t.Fatal(err)
			}
		...
		Assess("Namespace exists and has custom search attributes", func(ctx context.Context, _ *testing.T, cfg *envconf.Config) context.Context {
			// namespace := GetNamespaceForFeature(ctx)
			connectAddr, closePortForward, err := forwardPortToTemporalFrontend(ctx, cfg, t, cluster)
			if err != nil {
				t.Fatal(err)
			...
			// temporalClient, err := temporal.GetClusterClient(ctx, client, cluster)
			// if err != nil {
			// 	t.Fatal(err)
			// }
			...
			err = wait.For(func(ctx context.Context) (done bool, err error) {
				// If no error while describing the namespace, it works.
				_, err = nsClient.Describe(ctx, temporalNamespaceWithCustomSearchAttributes.GetName())
				if err != nil {
					var namespaceNotFoundError *serviceerror.NamespaceNotFound
					if errors.As(err, &namespaceNotFoundError) {
						return false, nil
				...
				// listRequest := &operatorservice.ListSearchAttributesRequest{Namespace: namespace}
				// serverSearchAttributes, err := temporalClient.OperatorService().ListSearchAttributes(ctx, listRequest)
				// if err != nil {
				// 	t.Fatal(err)
				// }
				// expectedCustomSearchAttributes, err := namespaceUtils.CreateIndexedValueTypeMap(&customSearchAttributes)
				// if err != nil {
				// 	t.Fatal(err)
				// }
				//
				// assert.Equal(t, expectedCustomSearchAttributes, serverSearchAttributes)
		...
		Assess("Namespace can be deleted", func(ctx context.Context, _ *testing.T, cfg *envconf.Config) context.Context {
			...
			err := retry.RetryOnConflict(retry.DefaultRetry, func() error {
				...
				temporalNamespace.Spec.AllowDeletion = true
				return cfg.Client().Resources(namespace).Update(ctx, temporalNamespaceWithCustomSearchAttributes)
			})
			if err != nil {
				t.Fatal(err)
			}
			...
			// Wait for controller to set finalizer.
			err = wait.For(func(ctx context.Context) (done bool, err error) {
				err = cfg.Client().Resources(namespace).Get(ctx, temporalNamespaceWithCustomSearchAttributes.GetName(), temporalNamespaceWithCustomSearchAttributes.GetNamespace(), temporalNamespaceWithCustomSearchAttributes)
				if err != nil {
					t.Fatal(err)
				}

				result := controllerutil.ContainsFinalizer(temporalNamespaceWithCustomSearchAttributes, "deletion.finalizers.temporal.io")
				return result, nil
			}, wait.WithTimeout(2*time.Minute), wait.WithInterval(1*time.Second))
			if err != nil {
				t.Fatal(err)
